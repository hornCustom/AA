...
import com.mobidever.database.base.SQLiteDALBase;
import com.mobidever.model.ModelCategory;
import com.mobidever.utility.DateTools;
...
public class SQLiteDALCategory extends SQLiteDALBase {
	...
	public Boolean UpdateCategory(String p_Condition, ModelCategory p_Info) {
		ContentValues _ContentValues = CreatParms(p_Info);
		return GetDataBase().update("Category", _ContentValues, p_Condition,
				null) > 0;
	}

	public Boolean UpdateCategory(String p_Condition,
			ContentValues p_ContentValues) {
		return GetDataBase().update("Category", p_ContentValues, p_Condition,
				null) > 0;
	}
	...
	protected ModelCategory FindModel(Cursor p_Cursor) {
		...
		_ModelCategory.SetCategoryID(p_Cursor.getInt(p_Cursor
				.getColumnIndex("CategoryID")));
		_ModelCategory.SetCategoryName(p_Cursor.getString(p_Cursor
				.getColumnIndex("CategoryName")));
		_ModelCategory.SetTypeFlag(p_Cursor.getString(p_Cursor
				.getColumnIndex("TypeFlag")));
		_ModelCategory.SetParentID(p_Cursor.getInt(p_Cursor
				.getColumnIndex("ParentID")));
		_ModelCategory.SetPath(p_Cursor.getString(p_Cursor
				.getColumnIndex("Path")));
		Date _CreateDate = DateTools.getDate(
				p_Cursor.getString(p_Cursor.getColumnIndex("CreateDate")),
				"yyyy-MM-dd HH:mm:ss");
		_ModelCategory.SetCreateDate(_CreateDate);
		_ModelCategory.SetState((p_Cursor.getInt(p_Cursor
	...
	private void InitDefaultData(SQLiteDatabase p_DataBase) {
		...
		_ModelCategory.SetTypeFlag(GetContext().getString(
				(R.string.PayoutTypeFlag)));
		_ModelCategory.SetPath("");
		_ModelCategory.SetParentID(0);
		String _InitDefaultCategoryNameArr[] = GetContext().getResources()
		...
		for (int i = 0; i < _InitDefaultCategoryNameArr.length; i++) {
			...
			p_DataBase.update("Category", _ContentValues, " CategoryID = "
					+ _NewID.intValue(), null);
		}
	}
	...
	public void OnCreate(SQLiteDatabase p_DataBase) {
		...
		s_CreateTableScript
				.append("				[CategoryID] integer PRIMARY KEY AUTOINCREMENT NOT NULL");
