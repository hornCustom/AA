...
public class SQLiteDALAccountBook extends SQLiteDALBase {
	...
	public Boolean InsertAccountBook(ModelAccountBook p_Info) {
		...
		Long p_NewID = GetDataBase()
				.insert("AccountBook", null, _ContentValues);
	...
	public Boolean UpdateAccountBook(String p_Condition, ModelAccountBook p_Info) {
		ContentValues _ContentValues = CreatParms(p_Info);
		return GetDataBase().update("AccountBook", _ContentValues, p_Condition,
				null) > 0;
	}

	public Boolean UpdateAccountBook(String p_Condition,
			ContentValues p_ContentValues) {
		return GetDataBase().update("AccountBook", p_ContentValues,
				p_Condition, null) > 0;
	}
	...
	protected ModelAccountBook FindModel(Cursor p_Cursor) {
		...
		_ModelAccountBook.SetAccountBookID(p_Cursor.getInt(p_Cursor
				.getColumnIndex("AccountBookID")));
		_ModelAccountBook.SetAccountBookName(p_Cursor.getString(p_Cursor
		...
		_ModelAccountBook.SetIsDefault(p_Cursor.getInt(p_Cursor
				.getColumnIndex("IsDefault")));
		_ModelAccountBook.SetState((p_Cursor.getInt(p_Cursor
	...
	private void InitDefaultData(SQLiteDatabase p_DataBase) {
		String _AccountBook[] = GetContext().getResources().getStringArray(
	...
	public void OnCreate(SQLiteDatabase p_DataBase) {
		...
		s_CreateTableScript
				.append("				,[AccountBookName] varchar(20) NOT NULL");
